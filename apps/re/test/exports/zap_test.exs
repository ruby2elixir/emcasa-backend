defmodule Re.Exporters.ZapTest do
  ...
  describe "build_xml/0" do
    test "export XML from listing" do
      ...
      %{id: id} = listing = insert(:listing,
        ...
        complement: "basement", images: images, description: "descr", area: 50, price: 1_000_000,
    ...
    test "export XML from listing with nil values" do
      ...
      %{id: id} = listing = insert(:listing,
        ...
        address: build(:address, city: "Rio de Janeiro", state: "RJ", neighborhood: "Copacabana",
          street: "Avenida Atlântica", street_number: "55", postal_code: "11111-111"),
        complement: "basement", images: images, description: nil, area: 50, price: 1_000_000,
        rooms: 2, bathrooms: 2, garage_spots: 1, maintenance_fee: 1000.00, property_tax: 1000.00)
      expected_xml = "<Imovel>" <>
        "<CodigoImovel>#{id}</CodigoImovel>" <>
        "<TipoImovel>Apartamento</TipoImovel>" <>
  ...
  describe "export_listings_xml/1" do
    test "should export listings wrapped" do
      ...
      assert "<?xml version=\"1.0\" encoding=\"UTF-8\"?><Carga xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"><Imoveis><Imovel><CodigoImovel>#{listing.id}</CodigoImovel></Imovel></Imoveis></Carga>"
