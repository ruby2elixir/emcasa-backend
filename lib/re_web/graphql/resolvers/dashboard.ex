defmodule ReWeb.Resolvers.Dashboard do
  ...
  def maintenance_fee_count(_params, _res) do
    {:ok, Repo.one(from l in Listing, where: not is_nil(l.maintenance_fee) and l.is_active == true, select: count(l.id))}
  end

  def property_tax_count(_params, _res) do
    {:ok, Repo.one(from l in Listing, where: not is_nil(l.property_tax) and l.is_active == true, select: count(l.id))}
  end

  def tour_count(_params, _res) do
    {:ok, Repo.one(from l in Listing, where: not is_nil(l.matterport_code) and l.is_active == true, select: count(l.id))}
  end

  def area_count(_params, _res) do
    {:ok, Repo.one(from l in Listing, where: not is_nil(l.area) and l.is_active == true, select: count(l.id))}
  end
